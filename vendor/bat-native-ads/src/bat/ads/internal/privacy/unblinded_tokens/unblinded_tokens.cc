...
void UnblindedTokens::SetTokensFromList(const base::Value& list) {
  ...
  for (const auto& value : list.GetList()) {
    ...
    ConfirmationType token_confirmation_type = ConfirmationType::kUndefined;
    AdType token_ad_type = AdType::kUndefined;
    ...
    if (value.is_string()) {
      ...
      // Ad type
      const std::string* ad_type = dictionary->FindStringKey("ad_type");
      if (ad_type) {
        token_ad_type = AdType(*ad_type);
      }
    }
    ...
    unblinded_token.confirmation_type = token_confirmation_type;
    unblinded_token.ad_type = token_ad_type;
